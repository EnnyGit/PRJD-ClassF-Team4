...
class ApiIntergration {
  ...
  Future<void> getRestingHeartRate() async {
    ...
    try {
      ...
      Response response = await get(
        Uri.parse("https://api.fitbit.com/1/user/-/activities/heart/date/today/$period.json"),
      ...
      for (var heartActivity in data['activities-heart']){
        restingHeartRate = heartActivity['value']['restingHeartRate'] == null ? restingHeartRate : heartActivity['value']['restingHeartRate'];
      }
    } 
    catch (e){
  ...
  Future<void> getSpeed() async{
    try {
      ...
      Response response = await get(
        Uri.parse("https://api.fitbit.com/1/user/-/activities/list.json?afterDate=2020-01-01&sort=desc&offset=0&limit=$limit"),
        headers: {"Authorization": "Bearer " + accessToken}
      );

      Map<String, dynamic> data = json.decode(response.body);

      speed = data['activities'][0]['speed'];
    } 
    catch (e){
      print('caught error: $e');
    }
  ...
  Future<void> getSleepScore() async {
    ...
    try {
      Response response = await get(
        Uri.parse("https://api.fitbit.com/1.2/user/-/sleep/list.json?afterDate=2020-01-01&sort=desc&offset=0&limit=1"),
        headers: {"Authorization": "Bearer " + accessToken}
      );
